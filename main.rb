...
post '/bet' do
  if params[:bet_amount].nil? || params[:bet_amount].to_i <= 0
    @error = "Must make a bet greater than 0, but less than $#{session[:player_pot]}."
    halt erb(:bet)
...
get '/game/compare' do
  ...
  if player_total < dealer_total
    ...
    winner!("#{session[:player_name]} hit backjack, and the dealer stayed at #{dealer_total}.")
  elsif player_total > dealer_total
    winner!("#{session[:player_name]} stayed at #{player_total}, and the dealer stayed at #{dealer_total}.")
  elsif dealer_total == BLACKJACK_AMOUNT && player_total < dealer_total
    loser!("#{session[:player_name]} stayed at #{player_total}, and the dealer hit Blackjack.")
  else
